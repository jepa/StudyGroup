...
# Tidiverse
Spain <- read_csv("./Data/SAU_Catch_Spain.csv")
head(Spain)
...
Separate_Example <- separate(Tidy_Spain,years,c("year", "month", "day"), sep = "-")
...
Selected_Species <- c("Mustelus asterias","Ophidiidae")
...
filter_starts_with_c <- filter(Spain, 
                               str_detect(common_name, 
                                          pattern="Lobster")
                               )
#OR return all results where the common name includes the string 'cod' OR 'crab'.
...
# Advanced example with ifelse

Example_ifelse <- Spain %>% 
  mutate(Is_it_Food = ifelse(end_use_type == "Direct human consumption","Yes","No!")) %>% 
  filter(Is_it_Food == "Yes")


Example_ifelse <- Spain %>% 
  mutate(Is_it_Food = ifelse(end_use_type == "Direct human consumption","Yes",
                             ifelse(end_use_type == "Other", "Maybe!",
                                    NA))) %>% 
  filter(!is.na(Is_it_Food))

### Multiple columns at a time

Example_Multiple <- Spain %>% 
  mutate_if(is.numeric,log) # mutate all of the columns with a rule

Example_Multiple <- Spain %>% 
  mutate_at(vars(tonnes:landed_value),mean)
...
Select_Example2 <- select(Spain, 4:7)

head(Select_Example2, 3)

# Drop column number 2 (area_type)
Select_Example2 <- select(Spain, -2)
...
# Example wioth numbers are characterss
...
#Now we bind the columns together
Bind_Row_1 <- bind_rows(Spain1,Spain2)
...
Diff_Species2 <- anti_join(Ireland, Senegal,
                           by=c("scientific_name","year")
                           )
...
Similar_Species <- semi_join(Ireland, Senegal,
                             by="scientific_name") %>% 
  pull(scientific_name) %>% 
  unique()
