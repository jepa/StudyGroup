...
for(i in 1:nrow(USArrests)){
  total.crimes[i] = USArrests$Murder[i] + USArrests$Assault[i] + USArrests$Rape[i]
}
total.crimes
```
...
for(i in 1:nrow(women)){
  BMI[i] = women$weight / (women$height * women$height)
}
BMI
```
...
cond = 3
while(cond <= 6){
  fibonnaci[cond] = fibonnaci[cond-1] + fibonnaci[cond-2]
  cond = cond+1
}
fibonnaci[6]
```
...
create_ratio_df = function(row = 3, col = 5){
  out = data.frame(matrix(NA, nrow = row, ncol = col))
  for(i in 1:row){
    for(j in 1:col){
      out[i , j] = i/j
    }
